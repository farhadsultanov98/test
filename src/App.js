...
import AdminChat from './components/admin-panel/chat/chat';
import AdminCompose from './components/admin-panel/compose/compose';

const RouterWrapper = () => {
  const location = useLocation();
  const isAdminRoute = location.pathname.startsWith('/admin');
  const isLoginRoute = location.pathname === '/loginpage';
  ...
  return (
    <>
      <Routes>
        ...
        {/* Admin Panel Routes */}
        <Route path="/loginpage" element={isAuthenticated ? <Navigate to="/admin/dashboard" /> : <LoginPage />} />
        ...
        <Route path="/admin/chat" element={isAuthenticated ? <AdminPanelLayout><AdminChat/></AdminPanelLayout> : <Navigate to="/admin" />} />
        <Route path="/admin/compose" element={isAuthenticated ? <AdminPanelLayout><AdminCompose/></AdminPanelLayout> : <Navigate to="/admin" />} />
      </Routes>
      {!isAdminRoute && !isLoginRoute && <Chat />}
    </>
  );
};
